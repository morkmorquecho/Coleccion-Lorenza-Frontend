  ...
  <div class="login-page">
    ...
    <div class="login-card">
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        <button class="social-btn" @click="loginWithGoogle" :disabled="loading">
        ...
        <button class="social-btn" @click="loginWithFacebook" :disabled="loading">
...
<script setup>
    ...
    const client = window.google.accounts.oauth2.initTokenClient({
      ...
      callback: async ({ error, access_token }) => {
        if (error) {
          ...
          message.value = 'No se pudo conectar con Google'
          return
        }
  ...
  const handleOAuthSuccess = async (googleToken) => {
    ...
    try {
      ...
      // Verificar si es string para mostrar en modal
      if (typeof err.context === 'string') {
        showModal.value = true
        message.value = err.context
      } else {
  ...
  const login = async () => {
    ...
    try {
      ...
      // Limpiar errores de campo anteriores
      errors.value = {}
      
      // Verificar el tipo de error
      if (typeof error.context === 'string') {
        // Error string → mostrar en modal
        showModal.value = true
        message.value = error.context
      } else if (error.context && typeof error.context === 'object') {
        // Error objeto → asignar a campos específicos
        errors.value = error.context
      } else {
        // Error desconocido
        console.log(errors.value)
        showModal.value = true
        message.value = 'Ocurrió un error inesperado. Por favor, intenta de nuevo.'
      }
    } finally {
      loading.value = false
    }
